...
//
exports.bingApi = function(hosts, concurrency, bingKey, callback) {
  var results = [];
  var q = async.queue(function (host, queueCallback) {
    var count = 50;
    var offset = 0;
    ...
    var options = {
      ...
      path: '/Data.ashx/Bing/Search/v1/Web' + params,
  ...
  var q = async.queue(function (host, queueCallback) {
    ...
    var req = http.request(options, function(res) {
      ...
      res.on('end', function() {
        ...
        if (urls) {
          urls.forEach(function(element) {
            element = element.match(/cite>(.*?)</i);
            if (element) {
              results.push({ ip: host,
                             name: [url.parse(element[1])['hostname']],
                             src: 'bing'
                           });
            }
          });

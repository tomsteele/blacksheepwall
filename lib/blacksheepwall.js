...
BSW.prototype.robtex = function(callback) {
  var self = this;
  // Loop through all hosts and identify unique C networks
  process.nextTick(function() {
    var cNetworks = [];
    for(var i = 0; i < self.hosts.length; i++) {
      var dup = false;
      var cnet = self.hosts[i].split(".", 3).join(".");
    ...
    performQueries(cNetworks);
  });
  // Perform each lookup of a cNetwork
  function performQueries(cNetworks) {
    async.eachLimit(cNetworks, self.concurrency, function(cnet, asyncCallback) {
      var d = domain.create();
      d.on('error', function(err) {
        if (typeof process.env.SHOW_BSW_ERRORS !== 'undefined') {
          winston.error(err.message);
      ...
      function handleResponse(res) {
        ...
        res.on('end', function() {
          // Search throw the returned HTML
                ...
                if (ipMatch) {
                  ...
                  self.hosts.forEach(function(h) {
                    if (ipaddr === h) {
                      ...
                      if (hn) {
                        self.results.push({ip: ipaddr, name: hn[1], src: 'robtex'});
                      }
                    }
                  });
                  prevIp = ipaddr;
                } else {
                  var dnsMatch = row.match(/<a href="\/dns\/([a-z0-9-\.]{2,}[a-z]).html"/i);
                  if (dnsMatch) {
                    var hn = dnsMatch[1];
                    self.hosts.forEach(function(h) {
                      if (prevIp === h) {
                        if (hn) {
                          self.results.push({ip: prevIp, name: hn, src: 'robtex'});
                        }
                      }
                    });
                  }
...
BSW.prototype.headers = function(callback) {
  ...
  function performRequest(host, protocol, asyncCallback) {
    var d = domain.create();
    d.on('error', function(err) {
      if (typeof proces.env.SHOW_BSW_ERRORS !== 'undefined') {
        winston.error(err.message);
      }
      asyncCallback();
    });

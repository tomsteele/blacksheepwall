...
exports.cert = function(hosts, concurrency, callback) {
  ...
  var q = async.queue(function (host, queueCallback) {
    ...
    cleartextStream.setTimeout(2000, function() { queueCallback(); });
            ...
            if (ipMatch) {
              ...
              if (dnsMatch) {
                ...
                hosts.forEach(function(h) {
                  if (prevIp === h) {
                    if (hn) {
                      results.push({ip: prevIp, name: hn, src: 'robtex'});
                    }
                  }
                });
              }
...
//
// parse http and https response headers
//
exports.headers = function(hosts, concurrency, callback) {
  var tasks = [];
  var results = [];

  function doRequest(host, proto, queueCallback) {
    if (proto === 'http') {
      var req = http.request({'host': host}, reqCallback);
    }
    else if (proto === 'https') { 
    ...
    function reqCallback(res) {
      res.on('end', function() {
        if (res.headers.location) {
          var lName = url.parse(res.headers.location)['hostname'];
          if (lName && lName.match(/[a-z]+/i)) {
            results.push( { ip: host,
                            name: lName,
                            src: proto + '-location'
                          });
  ...
  hosts.forEach(function(host) {
    tasks.push(function(taskCallback) {
      doRequest(host, 'https', function() {
        taskCallback(results);
      });
    });
    tasks.push(function(taskCallback) {
      doRequest(host, 'http', function(results) {
        taskCallback();
      });
    });
  });
  async.parallelLimit(tasks, concurrency, function(results) {
    callback(results);
  });
}
